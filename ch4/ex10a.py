...
class BinaryTree: 
    ...
    def checkEqual(self, root1,root2,out):
        if root1!=None and root2!=None: 
            if root1==root2:
                out=self.checkSubtree(root1.left, root2.left, out) and self.checkSubtree(root1.right, root2.right, out)
            else: 
                out=False
        return out
    
def checkSubtree(t1,t2):
    #bfs-go over each node x in larger t1 and check if its same as root ofsmaller t2
    # if it is,  then check if the subtree starting from that node x is same as t2 
    ...
    while(not q.isEmpty()):
        node=q.dequeue()
        if node==t2.root: 
            out=t1.checkEqual(node, t2.root, None)
            if out: 
                return True
            else: 
                if node.left!=None: 
